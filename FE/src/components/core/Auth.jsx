...
export const fetchUser = async (setCurrentUser, tabId = TAB_ID) => {
    ...
    if (storedUser) {
      try {
        ...
        if (userData && userData._id && userData.role) {
          console.log(
            `[Tab ${tabId}] fetchUser: Using cached data from localStorage`
          );
  ...
  const storedUser = localStorage.getItem(AUTH_STORAGE_KEY);
  console.log(
    `[Tab ${tabId}] fetchUser: storedUser from localStorage:`,
  ...
  if (storedUser) {
    try {
      const userData = JSON.parse(storedUser);
      console.log(
        `[Tab ${tabId}] fetchUser: Setting user from localStorage temporarily:`,
        userData
      );
      setCurrentUser(userData);
  ...
  try {
    ...
    if (response && response.data) {
      ...
      if (userData._id) {
        ...
        console.log(
          `[Tab ${tabId}] fetchUser: User data saved to localStorage`
        );
      } else {
        console.log(
          `[Tab ${tabId}] fetchUser: User data missing _id, clearing auth`
        );
        removeAuth();
        setCurrentUser(null);
        localStorage.removeItem(AUTH_STORAGE_KEY);
      ...
      console.log(
        `[Tab ${tabId}] fetchUser: No user data from API, clearing auth`
      );
      removeAuth();
      setCurrentUser(null);
      localStorage.removeItem(AUTH_STORAGE_KEY);
    ...
    if (error?.error === "USER_INACTIVE") {
      ...
      console.log(
        `[Tab ${tabId}] fetchUser: Clearing auth due to error:`,
        error.message
      );
      removeAuth();
      setCurrentUser(null);
...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (userData && userData._id && userData.role) {
          console.log(
            `[Tab ${TAB_ID}] AuthProvider: Found valid stored user data:`,
            userData
          );
          setCurrentUser(userData);
        } else {
          console.log(
            `[Tab ${TAB_ID}] AuthProvider: Invalid stored user data, removing...`
          );
          localStorage.removeItem(AUTH_STORAGE_KEY);
          localStorage.removeItem(AUTH_TIMESTAMP_KEY);
        }
        ...
        console.log(
          `[Tab ${TAB_ID}] AuthProvider: Error parsing stored user data:`,
          error
        );
        localStorage.removeItem(AUTH_STORAGE_KEY);
        localStorage.removeItem(AUTH_TIMESTAMP_KEY);
  ...
  useEffect(() => {
    ...
    console.log(
      `[Tab ${TAB_ID}] AuthProvider: currentUser changed:`,
      currentUser
    );
  }, [currentUser]);

  const detectAndLoadOrganization = async () => {
    try {
      const response = await instance.get(
        `/organization/user/${currentUser._id}`
      );
      if (response.data._id) {
        const orgData = response.data;
        const myRole = orgData.members?.find(
          (m) => m.user._id === currentUser._id
        )?.role;
        setRoleForOrg(myRole);
  ...
  const contextValue = {
    ...
    isAuthenticated: () =>
      !!(currentUser && currentUser._id && currentUser.role),
