...
import { EditOutlined, DeleteOutlined, UserOutlined, UndoOutlined } from "@ant-design/icons";
...
const UsersList = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await instance.get(`/user?page=${currentPage}&limit=${pageSize}`);
      return data.data;
    },
  });
      ...
      render: (services) => (
        <Space wrap size={[4, 4]}>
          {services?.map((service) => (
            <Tag key={service._id} color="purple">
              {service.name}
            </Tag>
          ))}
        </Space>
