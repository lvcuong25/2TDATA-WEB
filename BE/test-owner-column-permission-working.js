...
const TEST_USER_EMAIL = 'manager@test.com';
const TEST_USER_PASSWORD = 'Manager123';
...
async function testUserColumnPermission() {
  try {
    ...
    // Step 3: Get all column permissions (owner can view all)
    ...
    // Step 4: Get available targets
    console.log('\n🔍 Step 4: Getting available targets...');
    const availableTargetsRes = await axios.get(`${BASE_URL}/permissions/columns/${TEST_COLUMN_ID}/available-targets`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    
    console.log('✅ Available targets Status:', availableTargetsRes.status);
    console.log('✅ Available targets Data:', JSON.stringify(availableTargetsRes.data, null, 2));
