...
const TablePerm = new mongoose.Schema(
  { tableId: { type: mongoose.Schema.Types.ObjectId, ref: "Table", required: true }, create: Boolean, read: Boolean, update: Boolean, delete: Boolean },
  { _id: false }
);

// Column-level: visibility/edit/mask/deletable
const ColumnPerm = new mongoose.Schema(
  { tableId: { type: mongoose.Schema.Types.ObjectId, ref: "Table", required: true }, columnId: { type: mongoose.Schema.Types.ObjectId, ref: "Column", required: true }, columnKey: { type: String, required: true }, visibility: { type: String, enum: ["visible", "hidden"], default: "visible" }, edit: { type: String, enum: ["rw", "ro", "none"], default: "rw" }, deletable: { type: Boolean, default: false }, maskMode: { type: String, enum: ["plain", "masked"], default: "plain" } },
  { _id: false }
);

// Row-level: ABAC query template với placeholder $ctx.*
const RowPolicy = new mongoose.Schema(
  { tableId: { type: mongoose.Schema.Types.ObjectId, ref: "Table", required: true }, queryTemplate: { type: Object, required: true } },
  { _id: false }
);

// Cell-level rule locks: lock theo điều kiện (giống Protect range rules)
const CellRuleLock = new mongoose.Schema(
  { tableId: { type: mongoose.Schema.Types.ObjectId, ref: "Table", required: true }, where: { type: Object, required: true }, columns: { type: [String], required: true }, mode: { type: String, enum: ["readOnly", "hidden", "editableByRole", "editableByUser"], required: true }, roles: [{ type: mongoose.Schema.Types.ObjectId, ref: "BaseRole" }], users: [{ type: mongoose.Schema.Types.ObjectId, ref: "User" }] },
  { _id: false }
);
